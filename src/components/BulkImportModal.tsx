...
export default function BulkImportModal({ onClose }: BulkImportModalProps) {
  ...
  const [visitorPreview, setVisitorPreview] = useState<any[] | null>(null)
  ...
  // Handle file preview untuk visitors
  const handleVisitorFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (!file) return

    setVisitorFile(file)
    try {
      const { visitors: parsed } = await parseVisitorExcelFile(file)
      setVisitorPreview(parsed)
    } catch (error) {
      setMessage({ type: 'error', text: `Error parsing visitor file: ${(error as Error).message}` })
  ...
  const handleImport = async () => {
    if (!candidateFile && !visitorFile && !instrumentFile && !scheduleFile) {
    ...
    try {
      ...
      // Import Visitors
      if (visitorPreview && visitorFile) {
        try {
          const existingIds = new Set(visitors.map((i) => i.id))
          const newOnes = visitorPreview.filter((i) => !existingIds.has(i.id))
          if (newOnes.length > 0) {
            await bulkAddVisitors(newOnes)
            successCount++
          }
        } catch (error) {
          errors.push(`Visitor: ${(error as Error).message}`)
        }
      }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-screen overflow-y-auto">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          {message && (
            <div
              className={`md:col-span-2 p-4 rounded-lg text-sm ${
          ...
          {/* VISITORS SECTION */}
          <div className="border border-gray-200 rounded-lg p-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-3">🧑‍💼 Data Visitor</h3>
            <input
              type="file"
              accept=".xlsx,.xls"
            ...
            {visitorPreview && visitorPreview.length > 0 && (
              <div className="mt-3 overflow-x-auto">
                <p className="text-sm font-medium text-gray-700 mb-2">
                  Preview ({visitorPreview.length} data):
                </p>
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="bg-gray-100 border border-gray-200">
                  ...
                  <tbody>
                    {visitorPreview.slice(0, 3).map((item, idx) => (
                      <tr key={idx} className="border border-gray-200">
                        <td className="px-2 py-1">{item.full_name}</td>
                ...
                {visitorPreview.length > 3 && (
                  <p className="text-xs text-gray-500 mt-1">... dan {visitorPreview.length - 3} data lainnya</p>
                )}
              </div>
            )}
          </div>
          ...
          {/* SCHEDULES SECTION */}
          <div className="border border-gray-200 rounded-lg p-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-3">📅 Data Jadwal Home Visit</h3>
          <input
            type="file"
            accept=".xlsx,.xls"
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-gray-100 border border-gray-200">
                    ...
                    <th className="px-2 py-1 text-left">Etoser</th>
                ...
                <tbody>
                  {schedulePreview.slice(0, 3).map((item, idx) => (
                    <tr key={idx} className="border border-gray-200">
                      ...
                      <td className="px-2 py-1 text-xs">{item.etoser_id}</td>
      ...
      <div className="sticky bottom-0 border-t border-gray-200 p-6 flex gap-3 justify-end bg-gray-50 rounded-b-lg z-10">
        <button
          onClick={onClose}
        ...
        <button
          ...
          disabled={isProcessing || (!candidateFile && !visitorFile && !instrumentFile && !scheduleFile)}
