...
import { useVisitorStore, Visitor } from '../../store/visitorStore'
...
export default function DataVisitor() {
  ...
  const filteredVisitors = [...visitors].sort((a, b) => a.id.localeCompare(b.id))
                          ...
                          <button
                            onClick={() => {
                              setConfirmModal({
                                isOpen: true,
                                title: 'Hapus Visitor',
                                message: `Apakah Anda yakin ingin menghapus visitor ${visitor.full_name || visitor.name}? Data akan dihapus secara permanen.`,
                                onConfirm: async () => {
                        ...
                        <div key={visitor.id} className="bg-white border border-slate-100 rounded-xl p-4 shadow-sm space-y-3">
                          <div className="flex justify-between items-start">
                            <div className="flex-1 pr-2">
                              <p className="font-bold text-slate-800 text-base">{visitor.full_name || visitor.name}</p>
        ...
        return (
          <VisitorFormModal
            ...
            onSave={async (visitor) => {
              if (editingId) {
                await updateVisitor(editingId, { ...visitor, region: visitor.region || 'DKI Jakarta' })
                setToast({ message: 'Visitor updated successfully', type: 'success' })
                setEditingId(null)
                
              } else {
                await addVisitor({ ...visitor, region: visitor.region || 'DKI Jakarta' })
...
function VisitorFormModal({ visitor, suggestedId, onClose, onSave }: VisitorFormModalProps) {
  ...
  const [fullName, setFullName] = useState(visitor?.full_name || visitor?.name || '')
  const [role, setRole] = useState<'etoser' | 'mitra' | 'fasil'>(visitor?.role || 'etoser')
  const [region] = useState(visitor?.region || '')
